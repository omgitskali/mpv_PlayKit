...
LICENSE:
  ...
  --- azmr ver.
  https://github.com/azmr/colourblind/blob/master/LICENSE.md
...
// mat3 M_TritanopiaLMS = mat3( 1, 0, -0.86744736,
// 							 0, 1,  1.86727089,
// 							 0, 0,  1 );
...
vec4 hook() {
	...
	vec3 ImpairedC = linearize(color).rgb;

	if (MODE == 1) {
		ImpairedC = M_Protanopia * ImpairedC;
	} else if (MODE == 2) {
		ImpairedC = M_Deuteranopia * ImpairedC;
	} else if (MODE == 3) {
		ImpairedC = M_Tritanopia * ImpairedC;
	}

	vec4 fin = delinearize(vec4(ImpairedC, 1.0));
	return vec4(fin.rgb, color.a);
